...
async def make_lights(
    ...
    def _make_light(hass, light) -> MockEntity:
        identifier = "identifier-" + light
        device = device_registry.async_get_or_create(
            config_entry_id=config_entry.entry_id, identifiers={("light", identifier)}
        )

        entity_entry = entity_registry.async_get_or_create(
            "light", "", light, device_id=device.id
        )
        entity_id = entity_entry.entity_id

        entity_registry.async_update_entity(
            entity_id, area_id=area_name, device_id=device.id
        )
        return MockEntity(entity_id=entity_id, area_id=area_name)
